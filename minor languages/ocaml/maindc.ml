...
let register = Hashtbl.create 16
...
let print_number number =
	...
	let rec print_num pos =
		let res = len - pos in
		if (res >= 69) then (printf "%s\\\n%!" (String.sub num pos 69);
			print_num (pos + 69))
		else printf "%s\n%!" (String.sub num pos res)
	in print_num 0
...
let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' -> if (Hashtbl.mem register reg)
		then push (Hashtbl.find register reg) thestack
        | 's' -> Hashtbl.replace register reg (pop thestack)
