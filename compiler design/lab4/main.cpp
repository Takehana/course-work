...
void scan_opts (int argc, char** argv) {
   ...
   const char* filename = optind == argc ? "-" : argv[optind];
    ...
    filenameast = nameast;
    //rename sym
    string new_namesym = name_only + sym;
    char* namesym = new char[0];
    strcpy(namesym, new_namesym.c_str());
    filenamesym = namesym;
   cpp_popen (filename);
}
...
void fileout() {
    ...
    fclose(outstr);
    FILE* outsym = fopen(filenamesym, "w");
    //traverse(parser::root);
    scan_attr(outsym, parser::root);
